-- Adds an optional cover_key to the RPC. This is a new signature (one extra parameter), not just a
-- body change, so the old 5-argument overload has to be dropped explicitly — otherwise PostgREST
-- would see two candidate functions and refuse to pick one.
drop function if exists public.create_game_template(uuid, text, public.scoring_direction, jsonb, jsonb);
...
revoke all on function public.create_game_template(uuid, text, public.scoring_direction, jsonb, jsonb, text) from public, anon;
grant execute on function public.create_game_template(uuid, text, public.scoring_direction, jsonb, jsonb, text) to authenticated;
